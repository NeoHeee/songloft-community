...
class _LoginPageState extends ConsumerState<LoginPage> {
  ...
  List<DiscoveredServer> _discoveredServers = const <DiscoveredServer>[];
  ...
  Future<void> _discoverLanServers() async {
    ...
    try {
      ...
      setState(() {
        ...
        _discoveryMessage = wasCancelled
            ? '已停止搜索，保留当前发现结果'
            : results.isEmpty
  ...
  Future<void> _useDiscoveredServer(DiscoveredServer server) async {
    ...
    ResponsiveSnackBar.showSuccess(context, message: '已选择服务器 ${server.url}');
  }
  ...
  Widget _buildApiUrlField(ColorScheme colorScheme) {
    ...
    if (servers.length >= 2) {
      ...
      final selected = servers.any((server) => server.url == current)
          ? current
          : servers.first.url;
  ...
  Widget _buildServerDiscoveryControls(ColorScheme colorScheme) {
    ...
    final progress = _discoveryTotal <= 0
        ? null
    ...
    return Column(
      ...
      children: [
        OutlinedButton.icon(
          onPressed: _isDiscoveringServers
              ? _stopServerDiscovery
              : _discoverLanServers,
          icon: _isDiscoveringServers
              ? const Icon(Icons.stop_circle_outlined)
              : const Icon(Icons.radar_rounded),
          label: Text(_isDiscoveringServers ? '停止搜索' : '自动搜索局域网服务器'),
        ),
        ...
        if (_discoveryMessage != null) ...[
          ...
          Text(
            _discoveryMessage!,
            style: Theme.of(context).textTheme.bodySmall?.copyWith(
              color: _discoveredServers.isEmpty
