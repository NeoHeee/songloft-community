...
def patch_app_preferences() -> None:
    path = Path("lib/core/storage/app_preferences.dart")
    text = path.read_text(encoding="utf-8")

    if "_hiddenHomePluginEntriesKey" not in text:
        anchor = "  static const _sourcePlaylistIdKey = 'player_source_playlist_id';\n"
        if anchor not in text:
            raise RuntimeError("AppPreferences key anchor not found")
        text = text.replace(
            anchor,
            anchor
            + "  static const _hiddenHomePluginEntriesKey =\n"
            + "      'hidden_home_plugin_entries';\n",
            1,
        )

    if "getHiddenHomePluginEntries" not in text:
        marker = "  /// 清除所有偏好设置\n"
        if marker not in text:
            raise RuntimeError("AppPreferences method anchor not found")
        methods = dedent(
            """
              ...
              Set<String> getHiddenHomePluginEntries() {
                final entries =
                    _prefs.getStringList(_hiddenHomePluginEntriesKey) ??
                    const <String>[];
                return entries
...
def patch_home_greeting() -> None:
    path = Path("lib/features/home/presentation/home_page.dart")
    text = path.read_text(encoding="utf-8")

    already_patched = (
        "child: FittedBox(\n                        fit: BoxFit.scaleDown" in text
        or "child: FittedBox(\n              fit: BoxFit.scaleDown" in text
    )
    if already_patched:
        return
    ...
    start_marker = "                    Text(\n                      _greeting(),"
    end_marker = "                    const SizedBox(height: 8),\n"
    start = text.find(start_marker)
    end = text.find(end_marker, start)
    if start < 0 or end < 0:
        raise RuntimeError("Greeting block not found")
...
def write_plugin_grid() -> None:
    path = Path("lib/features/jsplugin/presentation/widgets/jsplugin_grid.dart")
    content = dedent(
        r"""
        ...
        class JSPluginGrid extends ConsumerWidget {
          ...
          Widget build(BuildContext context, WidgetRef ref) {
            ...
            final hiddenEntriesAsync = ref.watch(
              homePluginHiddenEntriesProvider,
            );
            ...
            return pluginsAsync.when(
              data: (plugins) {
                ...
                final hiddenEntries =
                    hiddenEntriesAsync.value ?? const <String>{};
                final visiblePlugins =
                    activePlugins
                        .where(
                          (plugin) =>
                              !hiddenEntries.contains(plugin.entryPath),
                        )
                        .toList();
...
def main() -> None:
    patch_app_preferences()
    patch_home_greeting()
    write_plugin_grid()
    print("Applied home mobile polish source changes")
